...
function buildHtml(leadName: string, landingUrl: string, appUrl: string): string {
  ...
  const pronounNom = gender === "female" ? "Pani" : "Pan";
  const pronounGen = gender === "female" ? "Pani" : "Pana";
  const pronounDat = gender === "female" ? "Pani" : "Panu";
...
<head>
  ...
  <style>
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .em-box td { background-color: #fff7f0 !important; }
          ...
          <tr>
            <td style="padding:0;line-height:0;">
              <a href="${landingUrl}" target="_blank" style="display:block;line-height:0;">
                <img src="${appUrl}/presale-video.gif" alt="Kliknij aby umówić spotkanie ▶" width="600" style="display:block;width:100%;max-width:600px;border:0;" />
              </a>
            </td>
          </tr>
...
export async function GET(
  ...
  try {
    ...
    const safeName = lead.full_name.replace(/[^a-zA-Z0-9_-]/g, "_");

    return new NextResponse(html, {
      headers: {
        "Content-Type": "text/html; charset=utf-8",
        "Content-Disposition": `attachment; filename="presale-${safeName}.html"`,
      },
    });
  } catch (err) {
