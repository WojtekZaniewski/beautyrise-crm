...
export default function SmsInboxPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`sms-inbox-convs:${workspaceId}`)
      .on(
        "postgres_changes",
  ...
  // Realtime: messages in open conversation
  useEffect(() => {
    if (!selectedConvId) return;
    const channel = supabase
      .channel(`sms-inbox-msgs:${selectedConvId}`)
      .on(
        "postgres_changes",
        ...
        <div className="flex-1 overflow-y-auto">
          {convLoading ? (
            ...
            <div className="p-6 text-center text-sm text-[var(--muted)]">
              {search ? "Brak wyników." : "Brak konwersacji.\nOdpowiedzi od klientów pojawią się tu automatycznie."}
            </div>
          ) : (
            filtered.map((conv) => {
        ...
        {!selectedConv ? (
          ...
          <>
            ...
            <div className="shrink-0 px-4 py-3" style={{ borderTop: "1px solid var(--border)", background: "var(--panel)" }}>
              {replyError && (
                <div className="text-xs text-red-500 mb-2">{replyError}</div>
              )}
